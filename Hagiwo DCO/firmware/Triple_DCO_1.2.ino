...
void setFreqs(Q16n16 freq) {
 ...
 mod_freq = ((carrier_freq >> 8)  * (mozziAnalogRead(1) / 2 +mozziAnalogRead(4) / 2));
 deviation = ((mod_freq >> 16) * (1 + mozziAnalogRead(3)+mozziAnalogRead(5))); // (Q16n16>>16)   Q8n8 = Q24n8, beware of overflow
...
void updateControl() {
  ...
  Mode_CV = map(mozziAnalogRead(2),0,1023,0,2);
  Gain_CV_0 = map(mozziAnalogRead(6),0,1023,0,1023);
  Gain_CV_1 = map(Gain_CV_0,0,1023,0,511);
  Gain_CV_2 = map(Gain_CV_0,0,1023,0,255);

  AnalogRead0 = mozziAnalogRead(0);
  AnalogRead1 = mozziAnalogRead(1);
  AnalogRead3 = mozziAnalogRead(3);
  AnalogRead4 = mozziAnalogRead(4);
  AnalogRead5 = mozziAnalogRead(5);
  AnalogRead7 = mozziAnalogRead(7);
  ...
  else if ((digitalRead(5) == true) && (digitalRead(4) == true)) {
    Switch = 1;
  }
...
//chord setting
 chord = (AnalogRead3 / 128) + (AnalogRead5 / 128);
 chord = constrain(chord, 0, 7);

 //inversion setting
 inv_knob = AnalogRead1;
 inv = (inv_knob  / 64)+ (AnalogRead4 / 64);
 ...
 if (inv_knob >= 1020) { //inv knob max
   wave = (AnalogRead3 / 128);
 }

 //frequency setting
 voct2 = AnalogRead7 ;
 freqv1 = freq1 * pow(2, (pgm_read_float(&(voctpow[voct2 + 205 * inv_aply1 + note1])))); //ROOT
 freqv2 = freq1 * pow(2, (pgm_read_float(&(voctpow[voct2 + 205 * inv_aply2 + note2])))); //2nd
 freqv3 = freq1 * pow(2, (pgm_read_float(&(voctpow[voct2 + 205 * inv_aply3 + note3])))); //3rd
 freqv4 = freq1 * pow(2, (pgm_read_float(&(voctpow[voct2 + 205 * inv_aply4 + note4])))); //4th
 freqv5 = freq1 * pow(2, (pgm_read_float(&(voctpow[voct2 + note5])))); //ROOT
...
if (Mode == 2) {
 ...
 return MonoOutput::from8Bit(aCarrier.phMod(modulation)*Gain_CV_1/256); // Internally still only 8 bits, will be shifted up to 14 bits in HIFI mode
 ...
 switch (wave) {
   ...
   case 0:
     return MonoOutput::from8Bit((aSaw1.next() / 8 + aSaw2.next() / 8 + aSaw3.next() / 8 + aSaw4.next() / 8 + aSaw5.next() / 8 * inv_aply5)*Gain_CV_0/256);
     break;

   case 1:
     return MonoOutput::from8Bit((aSqu1.next() / 8 + aSqu2.next() / 8 + aSqu3.next() / 8 + aSqu4.next() / 8 + aSqu5.next() / 8 * inv_aply5)*Gain_CV_0/256);
     break;

   case 2:
     return MonoOutput::from8Bit((aTri1.next() / 8 + aTri2.next() / 8 + aTri3.next() / 8 + aTri4.next() / 8 + aTri5.next() / 8 * inv_aply5)*Gain_CV_0/256);
     break;

   case 3:
     return MonoOutput::from8Bit((aSin1.next() / 8 + aSin2.next() / 8 + aSin3.next() / 8 + aSin4.next() / 8 + aSin5.next() / 8 * inv_aply5)*Gain_CV_0/256);
     break;

   case 4:
     return MonoOutput::from8Bit((aChb1.next() / 8 + aChb2.next() / 8 + aChb3.next() / 8 + aChb4.next() / 8 + aChb5.next() / 8 * inv_aply5)*Gain_CV_0/256);
     break;

   case 5:
     return MonoOutput::from8Bit((ahSin1.next() / 8 + ahSin2.next() / 8 + ahSin3.next() / 8 + ahSin4.next() / 8 + ahSin5.next() / 8 * inv_aply5)*Gain_CV_0/256);
     break;

   case 6:
     return MonoOutput::from8Bit((aSig1.next() / 8 + aSig2.next() / 8 + aSig3.next() / 8 + aSig4.next() / 8 + aSig5.next() / 8 * inv_aply5)*Gain_CV_0/256);
     break;

   case 7:
     return MonoOutput::from8Bit((aPha1.next() / 8 + aPha2.next() / 8 + aPha3.next() / 8 + aPha4.next() / 8 + aPha5.next() / 8 * inv_aply5)*Gain_CV_0/256);
     break;
